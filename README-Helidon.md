...
```
docker build -t SSP-API .
```

## Start the application with Docker

```
docker run --rm -p 8080:8080 SSP-API:latest
```
...
kubectl get service SSP-API  # Get service info
```
...
```
docker build -t SSP-API-native -f Dockerfile.native .
```

Start the application:

```
docker run --rm -p 8080:8080 SSP-API-native:latest
```